...
(function () {
	...
	Animation.prototype = {
		...
		parse :
		function () {
			this.parseArg(Array.prototype.slice.call(arguments, 0));
			//this.parseNonArrayArg(' ');
			delete this.notstart;
			if (this.compile() && this.elements.length) this.start();
		},
		
		parseArg :
		function (arg) {
			if (arg instanceof Array) for (var i = 0, n = arg.length; i < n; ++i) this.parseArg(arg[i]);
			else this.parseNonArrayArg(arg);
		},
			...
			switch (typeof(arg)) {
				case 'string':
					if (arg) {
						...
						if (o && typeof(o.style) == 'object') this.elements.push(o);
						else { this.args.push({p:'s', v:arg}); this.notstart = true; }
					}
					break
				
				case 'object':
					if (typeof(arg.style) == 'object') this.elements.push(arg);
					break;
					
				case 'number':
					if (!this.notstart) { this.millisec = arg; this.notstart = true; }
					else this.args.push({p:'m', v:arg});
					break;
				
				case 'function':
					this.args.push({p:arg.isEasing ? 'e' : 'c', v:arg});
					this.notstart = true;
					break;
			}
		},
		...
		function () {
			...
			if (z) var k = 1, d = this.millisec ? Math.max(0, Math.round((this.millisec - m) / z)) : 1000;
			else k = this.millisec ? this.millisec / m : 1, d = this.millisec || 1000;
